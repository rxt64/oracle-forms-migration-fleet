...
public static class MigrationLandscapeCatalog
{
    ...
    [
        ...
        new("aws-samples/sample-oracleforms-to-angular", ToolAuthority.WorkshopReference,
            "MIT-0 modernization sample with useful documentation patterns for staged extraction, traceable code generation, equivalence tests, optional shadow comparison, and separate pipeline and target architecture views. Its third-party notice attributes the included FMB and SQL artifacts to a separate MIT-licensed upstream project.",
            "Its six .fmb files are six modules, not Oracle Forms version 6. All inspected binaries start with ROS.60050, and the sample parser describes them as Forms 10g/12c object stores. That parser scans printable runs and associates nearby PL/SQL text with trigger markers; it is sample-specific heuristic extraction, not a general FMB decoder. The published target retains Oracle rather than proving an Azure PostgreSQL exit, so this remains workshop guidance and not end-to-end conversion proof.",
            "https://github.com/aws-samples/sample-oracleforms-to-angular"),
