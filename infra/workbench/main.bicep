...
@description('Dedicated non-human client applications permitted to call the workbench for deployment verification.')
...
@description('Existing Container Apps environment storage name for durable source and generated artifacts. Leave empty to retain replica-local storage.')
param workbenchArtifactStorageName string = ''

@description('Absolute container path for durable source workspaces and generated artifacts.')
param workbenchArtifactMountPath string = '/mnt/workbench-sources'
...
resource workbench 'Microsoft.App/containerApps@2025-01-01' = if (deployWorkbench) {
  ...
  properties: {
    ...
    template: {
      containers: [
        {
          ...
          env: [
            ...
            {
              name: 'WORKBENCH_SOURCE_ROOT'
              value: empty(workbenchArtifactStorageName) ? '/tmp/workbench-sources' : workbenchArtifactMountPath
            }
          ]
          volumeMounts: empty(workbenchArtifactStorageName) ? [] : [
            {
              mountPath: workbenchArtifactMountPath
              volumeName: 'workbench-artifacts'
            }
          ]
          probes: [
            {
      ...
      volumes: empty(workbenchArtifactStorageName) ? [] : [
        {
          name: 'workbench-artifacts'
          storageName: workbenchArtifactStorageName
          storageType: 'AzureFile'
        }
      ]
      // One replica remains the execution concurrency boundary. With environment storage configured,
      // source and generated bytes survive revision replacement; without it the explicit /tmp root
      // preserves the prior replica-local behavior.
      scale: {
        minReplicas: 1
        maxReplicas: 1
      }
      ...
      azureActiveDirectory: {
        ...
        validation: {
          ...
          defaultAuthorizationPolicy: {
            allowedApplications: union([entraClientId], validationClientApplicationIds)
            allowedPrincipals: {
